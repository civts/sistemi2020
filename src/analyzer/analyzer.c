 ...
 * Command:
 ...
 gcc -Wall -std=gnu90 analyzer.c ../common/parser.c ../common/mymath.c
 ../common/packets.c ../common/datastructures/fileList.c
 ../common/datastructures/miniQlist.c ../common/datastructures/namesList.c
 ../common/utils.c q.c p.c crawler.c miniQ.c controller.c  -o main -lm
...
string statuses[] = {"Still not started", "Analysis is running",
                     "Analysis finished"};

/*** Parameter for parser ****/
string argumentsAnalyzer[10];
int numberPossibleFlagsAnalyzer = 10;
string invalidPhraseAnalyzer =
    "Wrong command syntax, try command '-h' for help.\n";
string possibleFlagsAnalyzer[] = {"-analyze", "-i",    "-s",   "-h",
                                  "-show",    "-rem",  "-add", "-n",
                                  "-m",       "-quit", "-main"};
bool flagsWithArgsAnalyzer[] = {false, false, false, false, false, true,
                                true,  true,  true,  false, false};
bool settedFlagsAnalyzer[] = {false, false, false, false, false, false,
...
/**
 * Takes newCommand and puts it first in the history vector, moves previous
 * commands down.
 * No returns.
 */
void updateHistory(string newCommand) {
  ...
  for (i = HISTORY - 1; i > 0; i--) {
    strcpy(instanceOfMySelf.lastCommands[i],
           instanceOfMySelf.lastCommands[i - 1]);
  }
  strcpy(instanceOfMySelf.lastCommands[0], newCommand);
}

/**
 * Takes newMessage and puts it first in the history of messages vector, moves
 * previous messages down.
 * No returns
 */
void updateMessages(string newMessage) {
  int i;
  for (i = MESSAGES - 1; i > 0; i--) {
    strcpy(instanceOfMySelf.lastMessages[i],
           instanceOfMySelf.lastMessages[i - 1]);
  }
  strcpy(instanceOfMySelf.lastMessages[0], newMessage);
}

/**
 * Reads the first commands from the terminal, then switches to inputReader
 * cycle.
 * Return values:
 * 9 -> all good
...
int main(int argc, char *argv[]) {
  ...
  cleanArguments();
  bool validCall = checkArguments(
      argc - 1, argv + 1, possibleFlagsAnalyzer, flagsWithArgsAnalyzer,
      numberPossibleFlagsAnalyzer + 1, settedFlagsAnalyzer, argumentsAnalyzer,
      invalidPhraseAnalyzer, true);
...
/**
 * Sets mode to interactive, sends all present files to the controller, then
 * returns to readInput cycle.
 */
int interactiveMode() {
  int returnCode = 0;
...
// Generate an "empty" instance of controller, this method is to be used
// everytime.
int generateNewControllerInstance() {
  int returnCode = 0;
...
void printScreen() {
  ...
  //     instanceOfMySelf.lastMessages[i]);
  // }
  // printf("================================================\n");
  int i;
  for (i = 0; i < HISTORY; i++) {
...
int inputReader() {
  ...
  while (1) {
    if ((numReadCharacters =
             read(0, buf + lenBuffer, BUFFER_SIZE - lenBuffer)) > 0) {
      ...
      if (endCommandPosition != NULL) {
        ...
        bool validCall =
            checkArguments(numCommands, listOfCommands, possibleFlagsAnalyzer,
                           flagsWithArgsAnalyzer, numberPossibleFlagsAnalyzer,
                           settedFlagsAnalyzer, argumentsAnalyzer,
                           invalidPhraseAnalyzer, true);
  ...
  for (i = 0; i < numFiles; i++) {
    ...
    if (pathType == 0) {
      ...
      if (absolutePath != NULL) {
        ...
        // if(!instanceOfMySelf.hasMainOption){
        //     printf("Well, this is embarrassing... it seems we had problems
        //     checking file %s\n", fileNames[i]);
        // }
      }
    } else if (pathType == 1) {
 ...
 */
void sig_handler_A() { processExit(); }
  ...
  if (numBytesRead > 0) {
    if (packetHeader[0] == 16) {
      ...
      if (numBytesRead == 2 * INT_SIZE) {
        ...
        // if(!instanceOfMySelf.hasMainOption){
        //     printf("Something has gone wrong with a completedFile
        //     packet!\n");
        // }
        ret = 2;
      }
