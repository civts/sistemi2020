...
void miniQ(string fileName, miniQinfo *instanceOfMySelf) {
  ...
  if (instanceOfMySelf->index >= instanceOfMySelf->currM) {
    ...
    if (fileLength >= 0) {
      ...
      ull occurences[256];
      int numCharsInPortion =
          getBigOccurences(fileName, startPosition, endPosition, occurences);

      if (numCharsInPortion >= 0) {
        exitCode = sendOccurencesPacketToReport(
            instanceOfMySelf->pipeToQ, -1, instanceOfMySelf->fileId,
...
int sendErrorOnFileToReport(miniQinfo *instanceOfMySelf) {
  return reportErrorOnFilePacket(instanceOfMySelf->pipeToQ, -1,
                                 instanceOfMySelf->fileId);
}

// Get occurences of a portion of file given starting and ending position inside
// the file.
// If we get an error opening a file: return -1
int getBigOccurences(string fileName, long startPosition, long endPosition,
                     ull outOccurences[NUM_OCCURENCES]) {
  ...
  if (fd == -1) {
    ...
    if (lseek(fd, startPosition, SEEK_SET) == startPosition) {
      while (startPosition < endPosition) {
        long bufferSize =
            min_l(endPosition - startPosition, MINIQ_MAX_BUFFER_SIZE);
