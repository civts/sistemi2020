...
void waitForMessagesInPFromQ(pInstance *instanceOfMySelf) {
  ...
  for (i = 0; i < currM; i++) {
    ...
    if (numBytesRead == (1 + INT_SIZE)) {
      ...
      numBytesRead =
          read(qInstances[i].pipeQP[READ], packetData, dataSectionSize);
      // printf("Got packet %d in P from Q\n", packetHeader[0]);
      processMessageInPFromQ(packetHeader[0], packetData, dataSectionSize,
                             instanceOfMySelf);
    }
  }
}
...
void waitForMessagesInPFromController(pInstance *instanceOfMySelf) {
  ...
  numBytesRead =
      read(instanceOfMySelf->pipeCP[READ], packetHeader, 1 + INT_SIZE);
  ...
  if (numBytesRead == (1 + INT_SIZE)) {
    ...
    while (offset != dataSectionSize) {
      numBytesRead = read(instanceOfMySelf->pipeCP[READ], packetData + offset,
    ...
    processMessageInPFromController(packetHeader[0], packetData,
                                    dataSectionSize, instanceOfMySelf);
  }
}

int processMessageInPFromQ(byte packetCode, byte *packetData,
                           int packetDataSize, pInstance *instanceOfMySelf) {
  int returnCode;
  switch (packetCode) {
  case 6:
    returnCode =
        processPFileResults(packetData, packetDataSize, instanceOfMySelf);
    break;
  case 11:
    returnCode =
        processPErrorOnFilePacket(packetData, packetDataSize, instanceOfMySelf);
    break;
  default:
...
int processMessageInPFromController(byte packetCode, byte *packetData,
                                    int packetDataSize,
                                    pInstance *instanceOfMySelf) {
  int returnCode;
  switch (packetCode) {
  case 15:
...
int processPNewFilePacket(byte packetData[], int packetDataSize) {
  ...
  for (i = 0; i < currM; i++) {
    returnCode =
        forwardPacket(qInstances[i].pipePQ, 15, packetDataSize, packetData);
    if (returnCode < 0) {
...
int processPRemoveFilePacket(byte packetData[], int packetDataSize) {
  ...
  int i;
  for (i = 0; i < currM; i++) {
    if (forwardPacket(qInstances[i].pipePQ, 7, packetDataSize, packetData) <
        0) {
      returnCode = 1;
...
int processPFileResults(byte packetData[], int packetDataSize,
                        pInstance *instanceOfMySelf) {
  int returnCode = 0;
  int m = fromBytesToInt(packetData + 2 * INT_SIZE);

  if (currM == m) {
    if (forwardPacket(instanceOfMySelf->pipePC, 6, packetDataSize, packetData) <
        0) {
      returnCode = 1;
    }
  } else {
...
int processPErrorOnFilePacket(byte packetData[], int packetDataSize,
                              pInstance *instanceOfMySelf) {
  int returnCode = 0;

  if (forwardPacket(instanceOfMySelf->pipePC, 11, packetDataSize, packetData) <
      0) {
    returnCode = 1;
  }
...
int processPNewValueForM(byte packetData[], pInstance *instanceOfMySelf) {
  ...
  currM = newM;
  // Nota: qui non serve riassegnare i file perché C invia subito dopo il
  // pacchetto di cambio M anche
