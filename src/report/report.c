...
bool optionCombinationValid(bool *settedFlags) {
  ...
  if (settedFlags[help] + settedFlags[tab] + settedFlags[compact] +
          settedFlags[only] + settedFlags[verbose] >=
      2) {
    valid = false;
  }
  ...
  if (settedFlags[extended] &&
      (settedFlags[help] + settedFlags[tab] + settedFlags[compact] +
           settedFlags[only] + settedFlags[verbose] ==
       0))
    valid = false;
...
// funzione che fa il parsing dei file passati ad arguments trasformandoli in
// una lista di stringhe in resolvedPaths, lunghezza della lista in numArgs
bool parseArguments(char *arguments, int *numArgs, char **resolvedPaths) {
  ...
  *numArgs = i;
  // unresolvedPaths =
  // getArgumentsList(arguments[9],numFilesLog,unresolvedPaths);
  // i=0; j=0;
  // while(j<*numFilesLog){
  //     resolvedPaths[i]=realpath(unresolvedPaths[j],resolvedPaths[i]);
  //     if(inspectPath(resolvedPaths[i])!=-1){
...
int main(int argc, char *argv[]) {
  ...
  char *possibleFlags[] = {
      helpFlag,     verboseFlag,         tabFlag,  compactFlag, onlyFlag,
      extendedFlag, forceReAnalysisFlag, quitFlag, dumpFlag};
  int numberPossibleFlags = 9;
  // SPECIFICARE QUALI FLAG ACCETTANO ARGOMENTI, da passare in una stringa
  // "adasda dasdas asdad". Esempio: "--only "sdino ecmpa cobp""
  ...
  if (checkArguments(argc - 1, argv + 1, possibleFlags, flagsWithArguments,
                     numberPossibleFlags, settedFlags, arguments, NULL,
                     false)) {
    ...
    if (optionCombinationValid(settedFlags)) {
      ...
      if (settedFlags[only]) {
        ...
        if (parseArguments(arguments[only], &currentFilesCount,
                           resolvedPaths)) {
          // parsing riuscito
          valid = true;
          pathValid = true;
    ...
    while (1) {
      if ((numReadCharacters =
        ...
        if (endCommandPosition != NULL) {
          ...
          if (checkArguments(numCommands, spliced, possibleFlags,
                             flagsWithArguments, numberPossibleFlags, copyFlags,
            ...
            if (optionCombinationValid(copyFlags) && numCommands > 0) {
              ...
              if (copyFlags[only]) {
                // provo ad effettuare il parsing
                if (parseArguments(arguments[only], &tmpFilesCount,
                                   tmpResolvedPaths)) {
                  // parsing riuscito
                  valid = true;
                  pathValid = true;
                  ...
                  valid = false;
                  copyFlags[only] = false;
                  pathValid = false;
                }
              } else {
                valid = true;
              }
              // se ho un dump o un force, non cambio la modalità di
              // visualizzazione
              if ((copyFlags[dumps_idx] || copyFlags[force]) &&
                  (copyFlags[help] + copyFlags[tab] + copyFlags[compact] +
                       copyFlags[only] + copyFlags[verbose] ==
                   0)) {
                copyFlags[help] = settedFlags[help];
                copyFlags[tab] = settedFlags[tab],
                copyFlags[compact] = settedFlags[compact];
                copyFlags[only] = settedFlags[only];
                copyFlags[verbose] = settedFlags[verbose],
                copyFlags[extended] = settedFlags[extended];
              }
            }
          }
      ...
      if (settedFlags[only]) {
        ...
        printFilesOrFolder(analyzers, currentFilesCount, resolvedPaths,
                           !settedFlags[extended]);
      }
      if (settedFlags[help])
        printHelp();
      if (settedFlags[quit]) {
      ...
      if (analyzers->dumps)
        printf("Scrittura su file degli errori abilitata per tutte le nuove "
               "analisi\n");

      // NON ELIMINARE QUESTE DUE STAMPE BUF E FFLUSH, SONO FONDAMENTALI PER IL
